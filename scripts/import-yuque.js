...
function getDateTime(timeStr = null) {
	...
	if (timeStr) {
		...
		if (Number.isNaN(date.getTime())) {
			console.error(
				`错误：提供的时间格式无效 ${timeStr}，请使用 ISO 格式（如 2025-12-14T15:30:00）`,
			);
			process.exit(1);
		}
	} else {
...
function main() {
	const { filePath, title, tags, category, draft, lang, time, address } =
		parseArgs();

	if (!filePath) {
		console.error("错误：请提供语雀导出的 Markdown 文件路径");
		console.error(
			'示例：node scripts/import-yuque.js ~/Downloads/语雀文章.md --title "文章标题" --tags "前端,JavaScript" --category "技术"',
		);
		process.exit(1);
	}
